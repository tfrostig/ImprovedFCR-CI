...
wrap_qc <- function(x, lambda=0.4, ct, alpha_) {
  tryCatch({QC.CI(x, lambda, ct, alpha_) %>% unlist()},
           error = function(cond) {
             return(c(NA, NA))
           })
}
...
wrap_shortest <- function(x, ct, alpha_) {
  tryCatch({Shortest.CI(x, ct, alpha_) %>% unlist()},
           error = function(cond) {
             return(c(NA, NA))
           })
}
...
wrap_pratt <- function(x, r=1.2, ct, alpha_) {
  tryCatch({MP.CI(x, r, ct, alpha_) %>% unlist()},
           error = function(cond) {
             return(c(NA, NA))
           })
}
